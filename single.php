<?php include "sub-header.php" ?>
  ...
  <!-- 테마설명 -->
  <p class="theme-guide">
    해당 포스트는 관리자/글메뉴에서 편집하실 수 있습니다. <br>
  ...
  <ul class="post-nav">
    <li class="indicator"><!--이전글 썸네일-->
      <?php $prevPost = get_previous_post(true); if($prevPost) {?>
      <?php $prevthumbnail = get_the_post_thumbnail($prevPost->ID);?>
      <?php previous_post_link('%link',"$prevthumbnail <i class='fa-solid fa-circle-arrow-left'></i>", TRUE); ?>
      <?php } ?>
    </li>
    ...
    <li class="indicator"><!--다음글 썸네일-->
      <?php $nextPost = get_next_post(true); if($nextPost) { ?>
      <?php $nextthumbnail = get_the_post_thumbnail($nextPost->ID);?>
      <?php next_post_link('%link',"$nextthumbnail <i class='fa-solid fa-circle-arrow-right'></i>", TRUE); ?>
      <?php } ?>
    </li>  

    <li class="goto-list">
      <a href="<?php echo $categoryUrl; ?>"><i class="fa-solid fa-list"></i> 목록</a>
    </li>  
  </ul><!-- post-nav -->    
